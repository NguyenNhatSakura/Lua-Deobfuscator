...
function StripAst(ast) {
    ...
    stripStat(ast)
                          ...
                          // Thanks for the fucking warning herrit
function SolveMath(ast) { // This is some ugly code sorry for whoever is seeing this
                ...
                && expr.FunctionArguments.CallType === 'ArgCall') 
            {
                ...
                expr.FunctionArguments.ArgList.forEach((v, i) => {
                    ...
                    if (c !== undefined && (
                        ...
                        if (v) {
                            // this fucking sucks we cant replace the acctual statement- only the source that gets printed
                            // ok 10m later i have a work around- a very very shitty work around
                            // ok 20m later i have a better- still shit work around
                            ...
                            varsToReplace.push({
                                ...
                                Value: c,
                                Remove: () => {
                                    // i would have to create a whole ass thing where each thing is used and i really do cba so
                                    ...
                                    console.log("removing", v.var.Name)
                            ...
                            visitor.RepeatStat = (stat) => {
                                ...
                                if (condition.Type == "VariableExpr" && condition.Variable.Name == v.var.Name) {
                                    stat.Condition = c
                                    //removeArg()
                                }
                            }

                            visitor.AssignmentStat = (stat) => {
                                let lhs = stat.Lhs
                                let rhs = stat.Rhs
                                ...
                                for (let k = 0; k < rhs.length; k++) {
                                    ...
                                    if (l.Type == "VariableExpr" && l.Variable.Name == v.var.Name) {
                                        rhs[k] = c
                                        //removeArg()
                                    }
                                }
                            ...
                            // console.log(v.var.ReferenceLocationList)

                            /*v.var.RenameList.forEach(v => {
                                v(c.Token.Source, true)
                            })

                            // hey peak here these removes the args we "replace :/"
    ...
    // this is improve to acctually replace shit and not just what to print

    let getReplace = (Name) => {
        for (let i = 0; i < varsToReplace.length; i++) {
            let r = varsToReplace[i]
    ...
    visitor.RepeatStat = (stat) => {
        ...
        if (condition.Type == "VariableExpr") {
            let r = getReplace(condition.Variable.Name)

            if (r != undefined) {
                stat.Condition = r.Value
            }
        }
    }
    ...
    visitor.AssignmentStat = (stat) => {
        ...
        for (let k = 0; k < rhs.length; k++) {
            ...
            if (l.Type == "VariableExpr") {
                let r = getReplace(l.Variable.Name)
    ...
    visitor.LocalVarStat = (stat) => {
        let values = stat.ExprList
    ...
    visitor.IndexExpr = (stat) => {
        let index = stat.Index

        if (index.Type == "VariableExpr") {
            let r = getReplace(index.Variable.Name)
    ...
    visitor.BinopExpr = (stat) => {
        let lhs = stat.Lhs
        let rhs = stat.Rhs

        if (lhs.Type == "VariableExpr") {
            let r = getReplace(lhs.Variable.Name)
        ...
        if (rhs.Type == "VariableExpr") {
            let r = getReplace(rhs.Variable.Name)
...
luaminp.MinifyAst = function(ast, options) {
    let [glb, root] = AddVariableInfo(ast)
...
luaminp.BeautifyAst = function(ast, options) {
    let [glb, root] = AddVariableInfo(ast)
...
try {
    if (module != null && module.exports != null) {
        module.exports.Visit = VisitAst
        module.exports.Print = luaminp.Print
        module.exports.Parse = luaminp.Parse
        module.exports.SolveMath = SolveMath
        module.exports.Beautify = luaminp.Beautify
        module.exports.BeautifyAst = luaminp.BeautifyAst
        module.exports.Minify = luaminp.Minify
        module.exports.MinifyAst = luaminp.MinifyAst
